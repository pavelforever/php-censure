...
class Censure
{
     ...
     */
    //Если плохие слова есть, не обязательно вернёт tre, поэтому можно проверять только на === false
    public static function false_if_no_bad_words(
        $s,
        $delta = 3,
        ...
        $charset = 'UTF-8')
    {
        //Сначала моя проверка
        $badwords = 'сука,сучк,долбит,хуй,хуя,хуе,хер,целк,проститут,праститут,прастетут,прастетут,шлюх,говн,басран,бздение,бздеть,бздех,бзднуть,бздун,бздюха,бикса,блежник,блудилище,бляд,блябу,взье,еба , еба,взъе,взъё,волосянка,взебывать,выеть,говенка,говенный,говешка,гондон,гандон,дерьмо,дермо,долбоеб,дрисня,дрист,дроч,ебал,ебан,ебарь,выеб,ебатория,ебать,ебец,ебли,ебля,ебнут,ебня,ебун,елда,задрока,заеба,заебываться,заеть,залуп,замудохаться,засеря,засерать,засра,ебуч,кляпыжиться,курв,манда,мандей,мандеть,мандища,мандюк,минет,мокрощелка,мудак,муде,мудил,мудистый,мудня,мудоеб,мудозвон,муйня,наеб,наёб,обосра,отпороть,отъеть,охуительный,педрик,пердеж,пердение,пердеть,пердильник,перднуть,пердун,пердуха,пердь,пидор,пидар,пизд,разъе,разъе,срака,сраный,сранье,срать,хлюх,хуище,срун,уеть,хитрожо';//фик убрал
        $badwordsArr = explode(',', $badwords);
        foreach ($badwordsArr as $badword) {
            //не через str_replace, чтоб не все слова перебирало, а только до первого найденного!
            if(mb_stripos($s, $badword) !== false){
                return true;
            }
        }
        //УДАЛИТЬ СЛОВА, ВНУТРИ КОТОРЫХ ЕСТЬ ДРУГИЕ СЛОВА НАПРИМЕР, ЕСЛИ ЕСТЬ ПИДОРАС И ПИДОР, ПИДОРАС УДАЛИТСЯ
        /*$finalbadwordsArr = $badwordsArr;
        foreach ($badwordsArr as $badword) {
            foreach ($finalbadwordsArr as $key => &$badw) {
                if($badword === $badw) continue;
                if(strpos($badw, $badword) !== false){
                    unset($finalbadwordsArr[$key]);
                }
            }
        }
        echo implode(',', $finalbadwordsArr);
        die();*/
        //*******************************
        ...
        if ($re_badwords === null)
        {
            ...
            */
            ...
            ',
                ...
                #ЛОХ
                /*' л_?[оo]_?[хx]',*///закомментил, т.к. много комментариев, содержащих внутри лох, попадало под модерацию, например плохое
